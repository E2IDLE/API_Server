...
func (r *SessionRepository) Create(ctx context.Context, session *model.Session) error {
	_, err := r.pool.Exec(ctx,
		`INSERT INTO sessions (session_id, invite_code, status, sender_id, receiver_id, created_at, updated_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $7)`,
		session.SessionID, session.InviteCode, session.Status,
		session.SenderID, session.ReceiverID, session.CreatedAt, session.UpdatedAt,
	)
	return err
}
...
func (r *SessionRepository) FindByID(ctx context.Context, sessionID string) (*model.Session, error) {
	row := r.pool.QueryRow(ctx,
		`SELECT session_id, invite_code, status, sender_id, receiver_id, created_at, updated_at
		 FROM sessions WHERE session_id = $1`, sessionID,
	)
	s := &model.Session{}
	err := row.Scan(&s.SessionID, &s.InviteCode, &s.Status,
		&s.SenderID, &s.ReceiverID, &s.CreatedAt, &s.UpdatedAt)
...
func (r *SessionRepository) UpdateReceiver(ctx context.Context, sessionID, receiverID, status string) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE sessions SET receiver_id = $1, status = $2, updated_at = NOW()
		 WHERE session_id = $3`,
		receiverID, status, sessionID,
	)
	return err
}
...
func (r *SessionRepository) FindByUserIDPaginated(ctx context.Context, userID string, page, pageSize int) ([]model.Session, int, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.SessionID, &s.InviteCode, &s.Status,
			&s.SenderID, &s.ReceiverID, &s.CreatedAt, &s.UpdatedAt); err != nil {
			return nil, 0, err
		}
